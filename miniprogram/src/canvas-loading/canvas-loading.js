...
Component({
    ...
    methods: {
        ...
        async addToScene(list) {
            this.animatorList = this.animatorList.filter(v => v.name !== "animate");
            wx.showLoading({
                title: '加载中',
                mask: true
            })
            ...
            const promiseList = []
            if (list.type === 'text') {
                const p = xrframe.loadImageObject(this.scene, list, this.markerShadow, true, this)
                promiseList.push(p)

            } else if (list.type === "model") {
                const p = xrframe.loadModelObject(this.scene, list, true, this.markerShadow, this)
                promiseList.push(p)

            } else if (list.type === 'video') {
                const p = xrframe.loadVideoObject(this.scene, list, true, this.markerShadow, this)
                promiseList.push(p)

            } else if (list.type === 'screen' && this.template_type === "模版四") {
                const p = xrframe.loadImageObject(this.scene, list, this.markerShadow2, true, this)
                promiseList.push(p)
            } else if (list.type === 'screen') {
                const p = xrframe.loadImageObject(this.scene, list, this.markerShadow, true, this)
                promiseList.push(p)
            ...
            await Promise.all(promiseList).then(async results => {
                ...
                if (this.template_type !== "模版四") {
                    await xrframe.handleShadowRotate(this, this.template_type)
                }
                // if (template_type === "模版二") {
                await xrframe.addTemplateTextAnimator(this.template_type, this.scene, this)
                // }
                xrframe.stopAnimatorAndVideo(this, false)
                xrframe.startAnimatorAndVideo(this)
                wx.hideLoading()
